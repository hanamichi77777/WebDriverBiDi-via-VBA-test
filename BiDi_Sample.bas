...
' [FIX] hWnd is a window handle and must be LongPtr on 64-bit Office (callers pass 0,
' which coerces safely; a real HWND would have been truncated with the old Long type).
...
' --- 2. Wait process verification ---
    ...
    Set elms_title1 = .FindElements(By.xpath, "//a[contains(@href, '/n/') and @aria-label and @title]")
    ' Wait 4 seconds
    .Wait 4000
      
    ' [2nd time] Search article count with FindElements
    Set elms_title2 = .FindElements(By.xpath, "//a[contains(@href, '/n/') and @aria-label and @title]")
    
    ' [Verification of page load completion]
    Dim msgText As String, msgCaption As String
    If elms_title1.count <> elms_title2.count Then
      msgText = "Waited, but" & Chr(10) & Chr(10) & " - Initial article count: " & elms_title1.count & Chr(10) & " - Article count after 4 sec: " & elms_title2.count & Chr(10) & " therefore the wait time is insufficient."
      msgCaption = "Wait Insufficient"
      MESSAGEbox 0, msgText, msgCaption, MB_OK Or MB_ForeFront
    Else
      msgText = "Waited, but" & Chr(10) & Chr(10) & " - Initial article count: " & elms_title1.count & Chr(10) & " - Article count after 4 sec: " & elms_title2.count & Chr(10) & " therefore it waited as expected."
