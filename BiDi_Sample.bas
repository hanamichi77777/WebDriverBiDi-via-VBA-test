...
' SCENARIO :
'   Osaka Å® Bangkok, round trip, then click Search
' NOTE :
'   Google Flights uses React/Wiz-driven comboboxes that replace the <input>
'   element twice during activation. A stable input routine must:
'     Phase 0 : Activate field and poll until activeElement is the input
'     Phase 1 : Clear existing value (select Å® delete Å® forwardDelete Å® fallback)
'     Phase 1.5 : Detect React/Wiz double replacement and update e = activeElement
'     Phase 2 : Per-character insertText with selectionStart polling
'   This guarantees:
'     - No Ågfirst character swallowedÅh issue
...
' ========================================================================================
    ...
    With driver
        ...
        ' ==========================================
        ' STEP 1: Set Departure City - "Osaka"
        ' ==========================================
        ' ExecuteInputValueByXPath (useKeyEvents=True) now performs:
        '
        '   Phase 0: Poll until the field is truly active
        '            (activeElement === input AND selectionStart available)
        '
        '   Phase 1: Safe clearing of existing value
        '            select() or selectAll Å® delete Å® forwardDelete Å® fallback clear
        '
        '   Phase 1.5: Detect React/Wiz double replacement
        '              (Google Flights replaces the input twice)
        '              Å® update e = activeElement when the final input appears
        '
        '   Phase 2: Per-character insertText with selectionStart polling
        '            (each character is confirmed inserted before next)
        
        Dim depXPath As String
        depXPath = "(//input[contains(@aria-label, 'Where from')])[last()]"
        
        ' Clear pre-populated value and type new city
        bidi.ExecuteInputValueByXPath depXPath, "Osaka"
        
        ' Targets the visible listbox only (role=listbox + aria-hidden=false) generated by the SPA.
        ' Selects the option element containing "Osaka" after the suggestion list is fully rendered.
        Dim depSuggestXPath As String
        depSuggestXPath = "//*[@role='listbox' and not(@aria-hidden='true')]//li[@role='option' and contains(@aria-label, 'Osaka')][1]"
        bidi.ExecuteClickByXPath depSuggestXPath, 5000
        
        ' ==========================================
        ' STEP 2: Set Destination City - "Bangkok"
        ' ==========================================
        ...
        bidi.ExecuteInputValueByXPath destXPath, "Bangkok", 5000
        
        ' Click matching suggestion
        Dim destSuggestXPath As String
       destSuggestXPath = "//*[@role='listbox' and not(@aria-hidden='true')]//li[@role='option' and contains(@aria-label, 'Bangkok')][1]"
