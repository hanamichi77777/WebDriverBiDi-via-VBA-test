...
End Sub
' ========================================================================================
' SeleniumVBA BiDi 同時接続数 限界突破テスト (Corrected URL Edition)
' 狙い: 正しいページで、VBAのソケット管理能力の限界を叩き出す
' ========================================================================================
Public Sub Test_ConcurrencyLimitBreak_Final()
    ' --- 設定：まずは4窓、成功したら5窓、10窓へ！ ---
    Const NUM_WINDOWS As Integer = 10
    
    ' 【重要】ボタンが存在するカテゴリページをターゲットにします
    Const TARGET_URL = "https://webscraper.io/test-sites/e-commerce/ajax/computers/laptops"
    
    Dim drvs() As WebDriver: ReDim drvs(1 To NUM_WINDOWS)
    Dim bidis() As BiDiCommandWrapper: ReDim bidis(1 To NUM_WINDOWS)
    ...
    ' --- 1. 順次起動フェーズ ---
    Debug.Print "--- [" & NUM_WINDOWS & "窓] 順次起動開始 ---"
    For w = 1 To NUM_WINDOWS
        Set drvs(w) = New WebDriver: Set bidis(w) = New BiDiCommandWrapper
        drvs(w).StartChrome
        Dim caps As WebCapabilities: Set caps = drvs(w).CreateCapabilities
        caps.EnableBiDiMode
        drvs(w).OpenBrowser caps
        bidis(w).ConnectTo drvs(w).GetWebSocketUrl
        drvs(w).NavigateTo TARGET_URL
    ...
    ' --- 2. 個別レディチェック ---
    Debug.Print "--- 全ウィンドウの描画完了を待機 ---"
    For w = 1 To NUM_WINDOWS
        ' 今度は正しいページなので、ボタン「1」が必ず見つかるはずです
        If bidis(w).ExecuteIsElementVisible("//button[@data-id='1']", 15000) Then
    ...
    For cycle = 1 To 5
        For w = 1 To NUM_WINDOWS
            ...
            ' インターロック付きクリック
            bidis(w).ExecuteClickByXPath "//button[@data-id='" & p & "']", _
    ...
    Debug.Print "--- テスト正常終了 (Total: " & Int(Timer - startTime) & "s) ---"

EmergencyShutdown:
    If Err.Number <> 0 Then Debug.Print "!!! エラー発生: [Win" & w & "] " & Err.Description
    Debug.Print "クリーンアップを開始します..."
    For w = 1 To NUM_WINDOWS
        On Error Resume Next
        If Not bidis(w) Is Nothing Then bidis(w).Shutdown
        If Not drvs(w) Is Nothing Then drvs(w).Shutdown
        Set bidis(w) = Nothing: Set drvs(w) = Nothing
        DoEvents
    Next w
End Sub
