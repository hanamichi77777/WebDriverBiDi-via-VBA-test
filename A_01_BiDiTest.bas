...
' --- 1. Execute BiDi Commands ---
    ...
    bidi.ExecuteNavigateAndGetStatus url, False
...
'  ' Example patterns to block common image formats and ad services
  ...
  ' Departure: Tokyo
  bidi.ExecuteInputValueByXPath "//input[@name='from']", "東京", , False
  ' Arrival: Shinjuku
  bidi.ExecuteInputValueByXPath "//input[@name='to']", "新宿", , False
  ' Date/Time: Last train (using Japanese text in XPath)
  bidi.ExecuteClickByXPath "//label[text()='終電']", , False
  ' Display order: Lowest price
  bidi.ExecuteSelectValueByXPath "//select[@id='s']", "料金が安い順", True, , False
  ' Click search button
  bidi.ExecuteClickByXPath "//input[@id='searchModuleSubmit']", , False
...
Public Sub Main05()
  ...
  With driver
   ...
   Dim bidi As New BiDiCommandWrapper: bidi.ConnectTo .GetWebSocketUrl
    ...
    bidi.ExecuteNavigateAndGetStatus "https://www.yahoo.co.jp/", True
...
Public Sub Main07()
  ...
  ' Click
  bidi.ExecuteShadowClick path, 10000, True

  ' Input
  bidi.ExecuteInputValueByXPath "//input[@id='username']", "aaa", 10000, False
