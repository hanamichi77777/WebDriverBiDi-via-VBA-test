...
Public Sub Main01()
  ...
  ' Enable Stealth Mode (CDP)
   bidi.ExecuteEnableStealthMode
...
' --- 1. Execute BiDi Commands ---
    ...
    bidi.ExecuteSessionSubscribe eventsToSubscribe
...
' --- 2. Search for XPath element and execute click ---
    ...
    bidi.ExecuteSelectValueAndWaitByXPath strClickXpath, "2026年03月", True
...
'  ' Example patterns to block common image formats and ad services
  ...
  bidi.ExecuteNavigateAndGetStatus url, True, 200, 10000, , True

  ' Departure: Tokyo
  bidi.ExecuteInputValueAndWaitByXPath "//input[@name='from']", "東京"
  ' Arrival: Shinjuku
  .FindElement(By.xpath, "//input[@name='to']").SendKeys "新宿"
  ' Date/Time: Last train (using Japanese text in XPath)
  .FindElement(By.xpath, "//label[text()='終電']").Click
  ' Display order: Lowest price
   bidi.ExecuteSelectValueAndWaitByXPath "//select[@id='s']", "1", , False ' Lowest price
  ' Click search button
  bidi.ExecuteClickAndNavigateByXPath "//input[@id='searchModuleSubmit']", True, , , 200
...
' [BiDi Storage: Cookie Backup & Restore (Session Persistence)]
' This version suppresses all VBA-side Debug.Print logs.
Public Sub Main06()
  ...
  ' -----------------------------------------------------------
  Dim targetUrl As String: targetUrl = "https://hotel-example-site.takeyaqa.dev/ja/login.html"
  .NavigateTo targetUrl
    
  Dim msgText As String
  msgText = "Please log in manually." & vbCrLf & _
            "After logging in (when Top or MyPage appears), click OK."
  MESSAGEbox 0, msgText, "Login Wait", MB_OK Or MB_ForeFront
    
  ' -----------------------------------------------------------
  ' 2. Backup Cookies (Get via BiDi)
  ' -----------------------------------------------------------
  ' [LOG REMOVED] Debug.Print "--- [Backup] Getting all cookies via BiDi ---"
    
  Dim cookies As Collection
  Set cookies = bidi.ExecuteStorageGetCookies()
    
  If cookies.Count = 0 Then
      MsgBox "No cookies found. Script will stop.", vbExclamation
      .CloseBrowser: .Shutdown: Exit Sub
  End If
    
  ' [LOG REMOVED] Debug.Print "Captured Cookie Count: " & cookies.Count
   
  ' (Optional) Output specific cookie details -> REMOVED
    
  ' -----------------------------------------------------------
  ' 3. Simulate Logout (Delete All Cookies via BiDi)
  ' -----------------------------------------------------------
  msgText = "Cookies captured." & vbCrLf & _
            "Press OK to DELETE all cookies (Simulate Logout)."
  MESSAGEbox 0, msgText, "Step 2", MB_OK Or MB_ForeFront
    
  bidi.ExecuteStorageDeleteAllCookies
    
  ' Refresh to prove we are logged out
  .Refresh
  ' [LOG REMOVED] Debug.Print "--- Cookies deleted. Page refreshed. ---"
    
  msgText = "Cookies have been deleted." & vbCrLf & _
            "Verify that you are logged out, then click OK to RESTORE session."
  MESSAGEbox 0, msgText, "Step 3", MB_OK Or MB_ForeFront
    
  ' -----------------------------------------------------------
  ' 4. Restore Cookies (Set via BiDi)
  ' -----------------------------------------------------------
  ' [LOG REMOVED] Debug.Print "--- [Restore] Injecting cookies via BiDi ---"
    
  Dim restoreCount As Long: restoreCount = 0
  Dim cookieData As Dictionary
  Dim newCookie As Dictionary
   
  ' Helpers for Value/Structure
  Dim valParams As Dictionary
  Dim rawVal As String
  Dim valObj As Object
    
  For Each cookieData In cookies
      Set newCookie = New Dictionary
        
      ' --- 1. Name ---
      newCookie.Add "name", cookieData("name")
      
      ' --- 2. Value (Revert to Object structure) ---
      ' Chrome BiDi expects a BytesValue object: {"type": "string", "value": "actual_string"}
      If IsObject(cookieData("value")) Then
          Set valObj = cookieData("value")
          rawVal = valObj("value")
      Else
          rawVal = CStr(cookieData("value"))
      End If
      
      ' Construct the BytesValue object
      Set valParams = New Dictionary
      valParams.Add "type", "string"
      valParams.Add "value", rawVal
      newCookie.Add "value", valParams
      
      ' --- 3. Domain, Path ---
      newCookie.Add "domain", cookieData("domain")
      newCookie.Add "path", cookieData("path")
        
      ' --- 4. Expiry (Must be Integer) ---
      If cookieData.Exists("expiry") Then
          newCookie.Add "expiry", Int(CDbl(cookieData("expiry")))
      End If
      
      ' --- 5. Security Attributes ---
      Dim sSite As String
      sSite = ""
      If cookieData.Exists("sameSite") Then
          sSite = LCase(cookieData("sameSite"))
          newCookie.Add "sameSite", cookieData("sameSite")
      End If
      
      Dim isSecure As Boolean: isSecure = False
      If cookieData.Exists("secure") Then isSecure = cookieData("secure")
      
      ' Security Rule: If SameSite is "none", Secure MUST be true.
      If sSite = "none" Then isSecure = True
      
      newCookie.Add "secure", isSecure
      
      If cookieData.Exists("httpOnly") Then newCookie.Add "httpOnly", cookieData("httpOnly")
        
      ' Execute SetCookie
      bidi.ExecuteStorageSetCookie newCookie
      restoreCount = restoreCount + 1
  Next cookieData
    
  ' [LOG REMOVED] Debug.Print "Restored " & restoreCount & " cookies."
    
  ' -----------------------------------------------------------
  ' 5. Verify Restoration
  ' -----------------------------------------------------------
  ' Navigate to a page that requires login (MyPage)
  .NavigateTo "https://hotel-example-site.takeyaqa.dev/ja/mypage.html"
    
  msgText = "Cookies restored!" & vbCrLf & _
            "If MyPage is displayed correctly, the test is SUCCESS."
  MESSAGEbox 0, msgText, "Verification Complete", MB_OK Or MB_ForeFront
...
Public Sub Main07()
  ...
  ' Click
  bidi.ExecuteShadowClickAndNavigate Path, True, 300, 20000, , 15000

  ' Wait
  If Not bidi.ExecuteBiDiWaitUntilUrlContains("signon.servicenow.com", 20) Then
  End If

  ' Input
  bidi.ExecuteInputValueAndWaitByXPath "//input[@id='username']", "aaa", , , True, 300
...
Public Sub Main08()
  ...
  With driver
    '============================================
    '【ファイルの保存先の設定】※同じフォルダ内に保存フォルダ作成
    Dim strPath As String
    strPath = .ResolvePath(".\") & "\保存フォルダ"
    '保存フォルダがなければ作成する
    ...
    ' Navigate to Page
    Dim url As String: url = "https://www.rpachallenge.com"
    .NavigateTo url
          
    ' --- Execute BiDi Wait Logic ---
    bidi.ExecuteSetDownloadPath strPath
    ...
    ' DownLoad
    bidi.ExecuteClickAndWaitByXPath "//a[text()=' Download Excel ']"
    
    ' Wait for the download completion event
    Dim isSuccess As Boolean
    isSuccess = bidi.ExecuteWaitForDownloadCompletion(clickTime, 30)
    
    Dim msgText As String
    If isSuccess = True Then
        msgText = "Download completed!"
        MESSAGEbox 0, msgText, "Download completed!", MB_OK Or MB_ForeFront
    Else
        msgText = "failure or timeout: "
        MESSAGEbox 0, msgText, "failure or timeout: ", MB_OK Or MB_ForeFront
    End If
...
Sub main10()
    ...
    bidi.ExecuteSelectValueAndWaitByXPath "//select[@id='userSelector']", "1"
