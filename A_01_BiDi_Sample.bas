...
Public Sub Main08()
  ...
  With driver
   ...
   ' Browser startup settings
   Dim caps As SeleniumVBA.WebCapabilities: Set caps = .CreateCapabilities
   caps.SetCapability "unhandledPromptBehavior", "ignore" 'Display an alert
   ...
   .OpenBrowser caps
    ...
    'Enable our new Auto-Accept feature
    bidi.AutoAcceptAlerts = True
   
    .NavigateTo "https://www.selenium.dev/selenium/web/alerts.html"
   
    bidi.ExecuteClickByXPath "//a[@id='alert']", , False
    bidi.ExecuteClickByXPath "//a[@id='empty-alert']", , False
    
    bidi.AutoPromptText = "WebDriver BiDi"
    bidi.ExecuteClickByXPath "//a[@id='prompt']", , False
    
    'Confirm input. Normal operation if no highlight appears here.
    Debug.Assert .FindElement(By.xpath, "//div[@id='text']/p").GetText = "WebDriver BiDi"
